;(function(){
  ...
  .factory('libFactory', function($rootScope, FIREBASE_URL, $http, $location){
      ...
      function _libsUrl(id){
        if(id){
          return FIREBASE_URL + "/users/" + $rootScope.user.uid + '/books/'+ id + ".json?auth=" + $rootScope.user.token;
        }else{
          return FIREBASE_URL + "/users/" + $rootScope.user.uid + "/books.json?auth=" + $rootScope.user.token;
        }
      }
